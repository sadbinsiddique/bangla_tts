...
# TTS customize for Bangla language based on Coqui TTS 
vendor_tts_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "vendor_tts"))
if os.path.isdir(vendor_tts_path) and vendor_tts_path not in sys.path:
    sys.path.insert(0, vendor_tts_path)

import TTS  # pyright: ignore[reportMissingImports]
from TTS.config import load_config  # pyright: ignore[reportMissingImports]
from TTS.tts.models import setup_model as setup_tts_model  # pyright: ignore[reportMissingImports]
from TTS.tts.utils.synthesis import synthesis, transfer_voice, trim_silence  # pyright: ignore[reportMissingImports]
from TTS.utils.audio import AudioProcessor  # pyright: ignore[reportMissingImports]

from TTS.vocoder.models import setup_model as setup_vocoder_model  # pyright: ignore[reportMissingImports]
from TTS.vocoder.utils.generic_utils import interpolate_vocoder_input  # pyright: ignore[reportMissingImports]
